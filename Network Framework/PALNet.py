...
class PALNet(nn.Module):
    def __init__(self, number_of_classes):
        ...
        """
        self.b_pool2 = nn.MaxPool3d(kernel_size=2, stride=stride2)
        ...
        self.fc12 = nn.Conv3d(128, self.number_of_classes, 1, 1, 0)  # C_NUM = 12, number of classes is 12
    ...
    def forward(self, x_tsdf=None, x_depth=None, p=None):
        ...
        if x_tsdf is not None:
            x1_tsdf = self.b_pool1(x_tsdf.float())            # (BS, 16L, 120L, 72L, 120L)
            #x1_tsdf  = x_tsdf
        ...
        """if (x_tsdf is not None) and (x_depth is not None):
            ...
            print('Input is None')
            exit(0)"""#Copied from here
        x_3 = x_3_tsdf

        x_4 = F.relu(self.res3d_1(x_3) + x_3, inplace=True) #nn.Conv3d, nn.ReLU, nn.Conv3d, nn.ReLU, nn.Conv3d,

        x_5 = F.relu(self.res3d_2(x_4) + x_4, inplace=True) #nn.Conv3d, nn.ReLU, nn.Conv3d, nn.ReLU, nn.Conv3d,
        x_6 = F.relu(self.res3d_3(x_5) + x_5, inplace=True) #nn.Conv3d, nn.ReLU, nn.Conv3d, nn.ReLU, nn.Conv3d,

        x_6 = torch.cat((x_3, x_4, x_5, x_6), dim=1)

        x_6 = F.relu(self.conv4_1(x_6), inplace=True)#nn.Conv3d

        x_6 = F.relu(self.conv4_2(x_6), inplace=True) #nn.Conv3d


        y = self.fc12(x_6)  #nn.Conv3d(128, 12, 1, 1, 0)
        #y = self.softmax(y)
        # print 'SSC: y', y.size()
